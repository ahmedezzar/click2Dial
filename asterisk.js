...
browser.webNavigation.onCompleted.addListener(details => {
    if (details.frameId !== 0) {
        frameIds[details.tabId] = details.frameId;
    }
});

browser.runtime.onMessage.addListener((message, sender) => {
    if (message.type === 'executeScriptInIframe') {
        console.log('Received message from content script, executing script in iframe');
        browser.tabs.executeScript(sender.tab.id, {
            file: 'content-script.js',
            frameId: frameIds[sender.tab.id]
        });
    }
});
...
browser.runtime.onMessage.addListener(function(message, sender, sendResponse) {
    if (message.type === 'PHONE_NUMBER_CLICKED') {
        placingCall(message.number);
    }
});
...
function placingCall(selectedNumber) {
  console.log("ca marche ! " + selectedNumber);
